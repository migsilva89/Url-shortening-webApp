...
function removeErrLayout() {
  ...
  let addLinkWarning = document
    .getElementById('addLinkWarning')
...
function displayData(originalLink, link1, link2, link3) {
  ...
  for (let i = 1; i < arrOfLinks.length; i++) {
    dynamicContent.innerHTML += `
      <section id="dynamicContent" class="bg-slate-100 mx-6 md:mx-0">
        <div class="container mx-auto">
            <div class="bg-white grid md:flex md:justify-between mt-10 rounded-xl p-5">
                ...
                <div class="grid md:flex gap-10 ">
                    ...
                    <button id="btnCopy" data-clipboard-text="https://${arrOfLinks[i]}" class="mx-8 bg-cyann btn-copy text-white py-3 px-8 rounded-lg hover:bg-cyann2">Copy</button>
                </div>
            </div>
        </div>
      </section>
